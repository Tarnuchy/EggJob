...
Zachowania są porozdzielane w taki sposób, aby obiekty nie musiały dźwigać zbędnego balastu. Przykładowo, klasa `GroupMember` implementuje role i permissions zamiast pchać wszystkie akcje grupy do klasy `User`. Dzięki temu każda klasa implementuje tylko te funkcje, które są jej potrzebne i spełniona jest zasada ISP.
