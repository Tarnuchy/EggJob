...
describe('Task-group access and invitations (UC-16..UC-27)', () => {
  it('invites friend directly to group with permissions (UC-22)', () => {
    ...
    expect(invited.value.entities.invitations['inv-g-1']?.kind).toBe('task-group');
  });
  ...
  it('accepts group invitation and adds member (UC-24)', () => {
    ...
    expect(accepted.value.entities.taskGroups['grp-1']?.memberIds).toContain('usr-friend');
  });
  ...
  it('adds member directly and removes member (UC-26)', () => {
    ...
    expect(removed.value.entities.taskGroups['grp-1']?.memberIds).not.toContain('usr-friend');
  });
  ...
  it('member can leave group (UC-27)', () => {
    ...
    expect(left.value.entities.taskGroups['grp-1']?.memberIds).not.toContain('usr-friend');
  });
