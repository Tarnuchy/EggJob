...
describe('Task lifecycle and comments (UC-29..UC-31 + Comment)', () => {
  ...
  it('deletes task and removes it from group (UC-31)', () => {
    ...
    expect(deleted.value.entities.taskGroups['grp-1']?.taskIds).not.toContain('tsk-1');
  });
  ...
  it('adds comment to progress entry timeline', () => {
    ...
    expect(commented.value.entities.comments['com-1']?.message).toBe('Dobra robota');
    expect(commented.value.entities.progressEntries['ent-1']?.commentIds).toContain('com-1');
  });
