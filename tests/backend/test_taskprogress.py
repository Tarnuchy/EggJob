...
def test_TaskProgress_updateProgress(db_session, bingo_bundle):
    ...
    progress.updateProgress(db_session, 50, "w berlinie na ulicy leżało")
    ...
    progress.updateProgress(db_session, 5, "biegałem dziś 5 dni")
    ...
    progress.updateProgress(db_session, 1, "biegałem znowu")
    ...
    old_value = progress.value
    progress.updateProgress(db_session, 1, "wtf udało się", photoUrl="http://photo.com/happyface.jpg")
    ...
    #test w drugą stronę
    progress.updateProgress(db_session, -1, "wywalili mnie xd", photoUrl="http://photos.com/sadface.jpg")
    ...
    with pytest.raises(ValueError):
        progress.updateProgress(db_session, 1, "nie dodaje zdjęcia ez?")
    
    with pytest.raises(ValueError):
        progress.updateProgress(db_session, "jeden", "ez", photoUrl="http://photos.com/asdf.jpg")
