            ...
            openedGroup && selectedGroupId === openedGroupId ? (
              ...
              <ScrollView contentContainerStyle={styles.groupsContent} showsVerticalScrollIndicator={false}>
                ...
                <View style={styles.groupList}>{memberGroups.length === 0 ? <EmptyState icon="albums-outline" title={t('tasks.groups.emptyTitle')} message={t('tasks.groups.emptyMessage')} /> : memberGroups.map(({ id, group }) => (
                  <Pressable key={id} style={({ pressed }) => [styles.groupCard, selectedGroupId === id && styles.groupCardActive, pressed && styles.groupCardPressed]} accessibilityRole="button" accessibilityLabel={group.name} onPress={() => { setSelectedGroupId(id); const canManage = group.ownerUserId === currentUserId || group.memberRoles[currentUserId] === 'admin'; navigation.navigate(canManage ? 'EditGroup' : 'GroupTasks', { groupId: id }); }}>
